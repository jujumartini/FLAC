...
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bannerCommenter::banner("test",
                        bandChar = ":")
##::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
##                             test                             ::
##::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
bannerCommenter::block("test",
                       bandChar = ":")
###  test         
bannerCommenter::boxup("test",
                       bandChar = ":")
##::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
##                              test                             :
##::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
bannerCommenter::open_box("test",
                          bandChar = ":")
##::::::::
##  test  
##::::::::
bannerCommenter::section("test",
                         bandChar = ":")
###:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
###:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
###                                                                      :::
...
###:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
###:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
banner("R INFRASTRUCTURE",
       minHashes = 80,
       bandChar = ":")
banner("OPTIONS",
       bandChar = ":")
banner("SHAPE #2: VARIABLE CONSISTENCY",
       bandChar = ":")
banner("SHAPE #5: FLAC AIM SPECIFIC",
       bandChar = ":")
banner("PROJECT WRITE",
       bandChar = ":")
bannerCommenter::banner("PROCESS FUNCTIONS",
                        emph = TRUE,
                        leftSideHashes = 4,
                        minHashes = 80,
                        bandChar = "%")
...
###  TESTING  :::::::::::::::::::::::::::::::::::::::::::::::::::
block("IMAGE NUMBERS",
      rightSideHashes = 58 - str_length("CHANGES"),
      bandChar = ":")
example_cli_progress_step_keep_messages <- function() {
  
  # cli_progress_step() adds cli’s alert themes to the status messages 
  # (info, success or danger), * prints the duration of each step 
  # (by default), and * it keeps the messages on the screen after 
  # they are terminated.
  ...
  cli_progress_bar(
    type = "custom",
    total = length(vct_file),
    format =
      '{col_blue("i")} Cleaning file {cli::pb_current}/{cli::pb_total}: {file} [{cli::pb_elapsed_clock}] | {cli::pb_eta_str}',
    format_done = "{col_green({symbol$tick})} All files cleaned!\n",
    format_failed = "Error: File #{cli::pb_current}: {file}",
    clear = FALSE,
    auto_terminate = TRUE,
    .auto_close = TRUE
  )
  
  for (i in seq_along(vct_file)) {
    
    file <- 
      vct_file[i]
    
    # NOTICE HOW cli_progress_step() IS INSIDE THE LOOP! *********************
    # cli::cli_progress_step(
    #   msg = "#{i}: {file}...poop...",
    #   msg_done = "#{i}: {file}...poop...DONE",
    #   msg_failed = ""
    # )
    ...
    if (i == 5) {
      cli_warn(message = c("!" = "File #{i}: {file}: Something is wrong here"))
    }
    
    cli_progress_update()
    
    # cli_progress_message(msg = c("i" = "poop...poop2...DONE"))
...
example_cli_progress_step_keep_messages()

explore_flac_condition <- function() {
  tib_function_health <- 
    paste("S:/_R_CHS_Research/PAHRL/Student Access/0_Students/MARTINEZ/3_Projects/FLAC - Aim 2",
          "Aim2DataAudit.xlsx",
          sep = "/") %>% 
    read_xlsx()
  tib_function_health$last3 <- 
    tib_function_health$StudyID %>% 
    str_sub(start = 2,
            end = 4)
  tib_function_health <- 
    tib_function_health[!is.na(tib_function_health$StudyID), ]
  tib_function_health$HEALTHNAME %>% 
    unique()
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "Healthy"] <- 
    "1. Healthy"
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "Stroke"] <- 
    "2. Stroke"
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "PD"] <- 
    "3. PD"
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "MS"] <- 
    "4. MS"
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "Arthritis"] <- 
    "5. Arthritis"
  tib_function_health$HEALTHNAME[tib_function_health$HEALTHNAME == "Other Cat"] <- 
    "7. Other Cat"
  
  test <- 
    tib_function_health[order(tib_function_health$last3,
                              tib_function_health$FUNCTIONCAT,
                              tib_function_health$HEALTHNAME), ]
  test2 <- 
    tib_function_health[order(tib_function_health$FUNCTIONCAT,
                              tib_function_health$last3,
                              tib_function_health$HEALTHNAME), ]
  
  tib_function_health <- 
    paste("S:/_R_CHS_Research/PAHRL/Student Access/0_Students/MARTINEZ/3_Projects/FLAC - Aim 1",
          "Aim1DataAudit.xlsx",
          sep = "/") %>% 
    read_xlsx()
  tib_function_health$last3 <- 
    tib_function_health$SubID %>% 
    str_sub(start = 2,
            end = 4)
  tib_function_health <- 
    tib_function_health[!is.na(tib_function_health$SubID), ]
  tib_function_health$HEALTHNAME %>% 
    unique()
  tib_function_health$HEALTHNAME <- 
    tib_function_health$Condition
...
aim3_ag_hr_differences <- function() {
  
  fdr_load <- 
    "FLAC_AIM3_DATA/PILOT"
  ...
  vct_fpa_hr <- 
    dir_ls(
      ...
      recurse = TRUE,
      type = "file",
      regexp = "HeartRate RR.csv$"
    )
  
  read_heartrate <- function(fpa_hr) {
    fnm_hr <- 
      fpa_hr %>% 
      path_file() %>% 
      str_remove(pattern = "DLW_Pilot_") %>% 
      str_remove("1secHeartRate RR.csv$")
    df_hr <- 
      fread(
        file = fpa_hr,
        sep = ",",
  ...
  map_dfr(.x = vct_fpa_hr,
          .f = ~read_heartrate(.x))
  
  df_hr$timestamp %>% seconds()
  
}

str_character_to_character_comparison <- function() {
...
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
shape_ag_raw_flac_v4 <- function(fdr_read,
                                 fdr_write,
                                 fdr_project  = NULL,
...
shape_ag_sec_flac_v2 <- function(fdr_read,
                                 fdr_write,
                                 fdr_project = NULL,
...
merge_chamber_rmr_v6 <- function(fdr_read,
                                 fdr_write,
                                 fdr_project = NULL,
                                 fld_chm = "CHAMBER",
                                 fld_rmr = "RMR",
...
merge_noldus_v8 <- function(fdr_read,
                            fdr_write,
                            fdr_project = NULL,
                            fld_act = "NOLDUS_ACTIVITY",
                            fld_pos = "NOLDUS_POSTURE",
...
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
####%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
compute_acc_model_estimates_v1 <- function(fdr_read,
                                           fdr_write,
                                           fdr_project = NULL,
                                           folder = NULL,
                                           filter_sub = NULL,
...
fdr_vid_merged <-
  ...
  # "Colorado/Noldus Observer XT 14/2_Event Logs/2_merge"
fdr_processed <- 
  "./Colorado/UWM_Modified CO DATA/DOINT/Staudenmayer"
  # "./3_data/2_processed"
fnm_merged_all <-
  "merged_all.rds"
fnm_duration_all <- 
...
tib_mer_clean <- 
  ...
  rename(intensity_video = intensity) %>% 
  select(subject,
         datetime,
         behavior,
         contains("intensity_"))
...
tib_mer_thick <- 
  tib_mer_clean %>% 
  padr::thicken(interval = "1 min",
                colname = "datetime_thick",
                by = "datetime") %>% 
  group_by(datetime_thick) %>% 
  summarise(mode_intensity_video = statistical_mode(intensity_video),
            mode_chamber_absolute = statistical_mode(intensity_chamber_absolute),
            mode_chamber_relative = statistical_mode(intensity_chamber_relative),
            .groups = "drop") %>% 
  rename(datetime = datetime_thick)
  
t1 <- 
  full_join(tib_mer_clean,
            tib_mer_thick)
...
test <- 
  ...
  rename_with(.fn = ~ str_remove(.x,
                                 pattern = "intensity_"),
              .cols = !c(subject:datetime))

get_total_per_visit <- function() {}

nrw_mer_complete <- 
  tib_vid_mer_all %>% 
  tidyr::drop_na(!c(matches("^activity"),
                    matches("^comment"))) %>% 
  nrow()
...
tib_variable <- 
  ...
  select(subject,
         visit,
         datetime,
         matches(paste0("^", .variable))) %>% 
  tidyr::drop_na()

if (nrow(tib_variable) != nrw_mer_complete) {
  
  stop("SHIZ IS GOING DOWN YO!\n",
       "tib_variable length does not equal length of complete cases tib_merged.")
  
}

variable
tib_variable %>% 
  select(matches(paste0("^", .variable))) %>% 
  colnames()
...
starwars %>% 
  # group_by(species) %>% 
  my_summarise(group_var = species,
               summarise_var = c(mass, height))

value_transform <- function(string_vector) {
  if_else(is.na(string_vector),
          true = 0,
          false = 1,
          missing = NULL)
}
...
test
tib_variable %>% 
  pivot_wider(names_from = intensity_video,
              values_from = intensity_video,
              values_fn = value_transform)

if_else(condition = is.na(test$Light),
        true = 0,
        false = 1,
        missing = NULL)
group_by(subject,
         visit) %>% 
  summarise(across(.cols = !datetime,
                   .fns = sum))
...
test
  ...
  summarise(
    total = n(),
    sed_chamber_absolute   = sum(chamber_absolute == "Sedentary"),
    light_chamber_absolute = sum(chamber_absolute == "Light"),
    mvpa_chamber_absolute  = sum(chamber_absolute == "Mod-Vig"),
    total_chamber_absolute = sed_chamber_absolute + light_chamber_absolute + mvpa_chamber_absolute,
    sed_chamber_relative   = sum(chamber_relative == "Sedentary"),
    light_chamber_relative = sum(chamber_relative == "Light"),
    mvpa_chamber_relative  = sum(chamber_relative == "Mod-Vig"),
    total_chamber_relative = sed_chamber_relative + light_chamber_relative + mvpa_chamber_relative,
    sed_video   = sum(video == "Sedentary"),
    light_video = sum(video == "Light"),
    mvpa_video  = sum(video == "Mod-Vig"),
    total_video = sed_video + light_video + mvpa_video
  )
relocate(contains("total"),
         .after = subject)
...
test_mer <- 
tib_vid_mer_all %>% 
  select(study:behavior) %>% 
  filter(!is.na(posture)) %>% 
  pivot_wider(names_from = posture,
              values_from = posture) %>% 
  pivot_wider(names_from = behavior,
              values_from = behavior)
  summarise(
    codes_posture = unique(posture),
    codes_behavior = unique(behavior)
  )
...
tib_vid_mer_all %>% 
  mutate()
tib_vid_mer_all[complete.cases(tib_vid_mer_all), ]
